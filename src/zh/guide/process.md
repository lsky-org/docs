...
云处理功能是一个通过 url 参数实时修改照片样式并输出的功能，支持任何已适配的储存。处理后的图片会自动缓存在本地服务器中，并支持两种输出响应方式(流输出、X-Sendfile)。
...
要使用云处理，需要先创建云处理驱动器，设置一个文件缓存目录后，将任意一个储存关联该云处理驱动器即可，该储存中的照片都可以使用云处理。

::: warning 请谨慎启用云处理功能
云处理对于照片有着较大的权限，容易被有心人利用，比如使用随机参数请求图片，可能会造成系统频繁的处理图片，占用系统资源。
:::

## 使用 X-Sendfile

使用 X-Sendfile 能够极大的提高照片访问速度，当前仅支持 Nginx，并且 nginx 需要安装 `ngx_http_sendfile_module` 模块。
